...
public abstract class FileOpsServiceBase extends IntentService
{
    ...
    public static void secureSendFile(Context context, Location srcLocation, Collection<? extends Path> paths, String passphrase, List<String> publicKeys)
    {
        Intent i = new Intent(context, FileOpsService.class);
        i.setAction(ACTION_SECURE_SEND);
        LocationsManager.storePathsInIntent(i, srcLocation, paths);
        i.putExtra("passphrase", passphrase);
        if (publicKeys != null) {
            i.putStringArrayListExtra("public_keys", new ArrayList<>(publicKeys));
        }
        context.startService(i);
    }
	...
	protected static final String ACTION_SECURE_SEND = "secure_send";
	...
	protected Task getTask(Intent intent)
	{
		switch (intent.getAction())
		{
			...
			case ACTION_SECURE_SEND:
				return new SecureSendTask();
